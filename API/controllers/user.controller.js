...
exports.register = async (req, res, next) => {
    try{
        const { username, email, password, gender, defAddress, favCategories, birthDate, role, status } = req.body;
        if ( !username || !email || !password || !gender || !birthDate){
            return res.status(400).json({ message: 'Hiányzó adatok!'});
        }
        const user = await userService.registerUser(username, email, password, gender, defAddress, favCategories, birthDate, role, status);
